...
// Add new node into given position
void LinkedList::add(double value, int pos) {
...
void LinkedList::removeVal(double value) {
...
void LinkedList::sortAsc() {
  // If list is empty or had one node, do nothing
  if (head == nullptr || head->next == nullptr)
    return;
  ...
  while (swap) {
    ...
    if (first->entry < second->entry) {
      ...
      if (first == nullptr) // Return if only two nodes
        return;
    } else {
    third = second;
    second = first;
